...
func Log(log logger.ILogger) func(next Handler) Handler {
	return func(next Handler) Handler {
		return HandlerFunc(
			func(w http.ResponseWriter, r *http.Request) error {
				...
				err := next.ServeHTTP(&respWriter, r)

				timeTaken := time.Since(startAt).String()

				commonLogs := map[string]any{
				...
				if err != nil {
					log.Error(ctx, fmt.Sprintf("[in-http] %s %s%s %d in %s",
						r.Method, r.Host, r.RequestURI, respWriter.statusCode, timeTaken),
						logger.WithFields(commonLogs),
					...
					//	logger.WithField("request", requestLog),
					//	logger.WithField("response", responseLog),
				}
